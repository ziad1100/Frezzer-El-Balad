  ...
  return (
    <div className="container-px py-16">
      <div className="mb-10 text-center">
        <h1 className="text-4xl font-extrabold text-[var(--tw-text)]">{t('nav.blog')}</h1>
      </div>
      {isLoading ? (
            ...
            <Link key={post._id} to={`/blog/${post.slug}`} className="group">
              <Card className="h-full transition-all duration-300 group-hover:-translate-y-1 group-hover:border-[var(--tw-border-strong)]">
                <CardContent className="flex h-full flex-col p-6">
                  ...
                  <h3 className="line-clamp-2 text-lg font-bold text-[var(--tw-text)] group-hover:text-brand-500">
                    {lang === 'ar' ? post.title : post.titleEn || post.title}
                  </h3>
                  <p className="mt-2 line-clamp-3 flex-1 text-sm text-[var(--tw-text-muted)]">
