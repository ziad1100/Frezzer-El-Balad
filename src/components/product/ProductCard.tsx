...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="group relative flex cursor-pointer flex-col overflow-hidden rounded-2xl border border-[var(--tw-card-border)] bg-[var(--tw-card-bg)] transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/10 hover:border-[var(--tw-border-strong)]">
      <Link to={`/product/${product.slug}`} className="flex flex-1 flex-col">
        ...
        <div className="relative aspect-[4/3] overflow-hidden bg-[var(--tw-surface-alt)]">
          {image ? (
            ...
            <div className="flex h-full w-full items-center justify-center">
              <ShoppingBag className="h-14 w-14 text-[var(--tw-text-subtle)]" />
            </div>
          )}
          {/* Overlay on hover */}
          <span
            aria-hidden="true"
            className="absolute inset-0 flex items-center justify-center bg-gradient-to-t from-black/40 to-transparent opacity-0 transition-all duration-300 group-hover:opacity-100"
          >
          ...
          <button
            ...
            className={cn(
              'absolute inset-e-3 top-3 flex h-9 w-9 items-center justify-center rounded-full backdrop-blur-sm transition-all duration-200',
              isWished ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/30' : 'bg-black/20 text-white hover:bg-brand-500',
            )}
            aria-label="wishlist"
          >
        ...
        <div className="flex flex-1 flex-col gap-1.5 p-4">
          <h3 className="line-clamp-1 text-sm font-bold text-[var(--tw-text)] transition-colors group-hover:text-brand-500">
          ...
          {product.labels && product.labels.length > 0 ? (
            <div className="flex flex-wrap gap-1">
              {product.labels.slice(0, 3).map((lbl) => (
                <span
                  key={lbl._id}
                  className="inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-[10px] font-semibold leading-tight"
                  style={{
              ...
              {product.labels.length > 3 && (
                <span className="inline-flex items-center rounded-full bg-[var(--tw-surface-alt)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--tw-text-subtle)]">
                  +{product.labels.length - 3}
                </span>
              )}
            </div>
          ...
          {product.reviewsCount > 0 ? (
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-[var(--tw-text-subtle)]">({product.reviewsCount})</span>
            </div>
          ) : (
            <p className="text-xs text-[var(--tw-text-subtle)]">{commonT('review.noReviews')}</p>
          )}
          <p className="line-clamp-1 text-xs text-[var(--tw-text-muted)]">
      ...
      <div className="px-4 pb-4">
        <div className="flex items-center justify-between pt-1">
          <div>
            ...
            <span className="ms-1 text-xs text-[var(--tw-text-subtle)]">{commonT('common.from')}</span>
          </div>
          <div className="flex items-center gap-1">
            <button
              onClick={handleSub}
              disabled={inCartQty === 0}
              className="flex h-8 w-8 items-center justify-center rounded-xl border border-[var(--tw-border-strong)] text-[var(--tw-text-muted)] transition-all hover:border-brand-500/50 hover:text-brand-500 disabled:cursor-not-allowed disabled:opacity-30"
            ...
            <button
              onClick={handleAdd}
              className="flex h-8 w-8 items-center justify-center rounded-xl bg-brand-500 text-white transition-all hover:bg-brand-600 hover:shadow-md hover:shadow-brand-500/20"
