...
export function ProductSelect({ value, onSelect, onClear, placeholder, disabled, className }: ProductSelectProps) {
  ...
  const dropdown = isOpen ? createPortal(
    <div
      ref={listRef}
      className="fixed z-[9999] max-h-72 overflow-y-auto rounded-xl border border-night-700 bg-night-900 shadow-2xl"
      style={{
      ...
      {value && !isOpen ? (
        ...
        >
          ...
          {onClear && (
            <button
              onClick={handleClear}
              className="text-night-500 hover:text-night-300 p-1"
              type="button"
            >
              <X className="h-4 w-4" />
            </button>
